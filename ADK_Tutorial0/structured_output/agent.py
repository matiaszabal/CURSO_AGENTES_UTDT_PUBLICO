...
# Estos modelos Pydantic definen el "contrato" de salida de cada agente: el LLM
# no puede devolver texto libre, tiene que completar exactamente estos campos.
...
# output_schema=ProblemAnalysis fuerza a que la respuesta de ESTE agente sea JSON
# válido contra ese modelo. output_key="problem_analysis_result" guarda ese JSON
# en el estado compartido de la sesión, con esa clave.
...
# input_schema=ProblemAnalysis: este agente espera recibir un JSON con esa forma
# (el {problem_analysis_result} en el instruction de arriba lo interpola desde el
# estado que dejó el agente anterior). output_schema=ConsultationResp fuerza,
# de nuevo, la forma de LA RESPUESTA. Así se encadenan dos agentes con contratos
# de datos tipados en vez de texto libre.
...
# SequentialAgent ejecuta sus sub_agents en orden, uno después del otro,
# compartiendo el mismo estado de sesión (por eso el segundo puede leer
# lo que el primero guardó en output_key).
